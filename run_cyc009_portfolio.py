# -*- coding: utf-8 -*-
"""
run_cyc009_portfolio.py — CYC-009 Cap-Tier Portfolio Model Batch Run
=====================================================================
Runs the same 111 CYC-008 factors through 4 market-cap quartile tiers
using point-in-time percentile breakpoints (adapts across 30 years).

Cap Tiers (point-in-time quartiles of valid universe each period):
  Small_Cap  — bottom quartile (0–25th percentile)
  Mid_Cap    — second quartile (25–50th percentile)
  Large_Cap  — third quartile  (50–75th percentile)
  Mega_Cap   — top quartile    (75–100th percentile)

Portfolio config (identical to CYC-008):
...
Usage:
    python run_cyc009_portfolio.py                      # full run, 4 tiers × 111 factors
    python run_cyc009_portfolio.py --dry-run             # 3 factors × 4 tiers, no save
    python run_cyc009_portfolio.py --tier Mega_Cap        # single tier only
    python run_cyc009_portfolio.py --factor jcn_qarp      # single factor, all tiers
    python run_cyc009_portfolio.py --min-obq 0.5          # only factors OBQ >= 0.5
"""
...
# All 4 cap tier labels
CAP_TIERS = ["Small_Cap", "Mid_Cap", "Large_Cap", "Mega_Cap"]

# ── Score direction registry (same as CYC-008) ───────────────────────────────
...
# ── Display name registry (same as CYC-008) ──────────────────────────────────
DISPLAY_NAMES = {
...
def _get_factor_list(bank_db: str, min_obq: float = 0.0,
                     factor_filter: str | None = None) -> list[dict]:
    """Load unique factors from factor bank."""
...
def _print_banner(n_factors: int, tiers: list[str], cfg: dict, dry_run: bool = False):
    try:
        dev = cp.cuda.Device()
    ...
    total_jobs = n_factors * len(tiers)
    print(f"\n{'='*70}")
    print(f"  CYC-009 Cap-Tier Portfolio Model Batch Run")
    print(f"  GPU:      {gpu_str}")
    print(f"  Factors:  {n_factors}")
    print(f"  Tiers:    {', '.join(tiers)} ({len(tiers)} tiers)")
    print(f"  Total:    {total_jobs} portfolio models")
    ...
    print(f"  Cap defs: Point-in-time quartiles (adaptive)")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="CYC-009 Cap-Tier Portfolio Model Batch Run")
    ...
    ap.add_argument("--hold",      type=int,   default=3,       help="Data pack hold_months (3mo quarterly)")
    ...
    ap.add_argument("--tier",      default=None,                 help="Run single tier only (Small_Cap, Mid_Cap, Large_Cap, Mega_Cap)")
    args = ap.parse_args()
    ...
    if args.tier:
        if args.tier not in CAP_TIERS:
            log.error(f"Invalid tier: {args.tier}. Must be one of: {CAP_TIERS}")
            sys.exit(1)
        tiers_to_run = [args.tier]
    else:
        tiers_to_run = CAP_TIERS
    ...
    # Filter factors to those in VRAM
    ...
    _print_banner(len(factors_in_vram), tiers_to_run, cfg, dry_run=args.dry_run)

    if not factors_in_vram:
        log.error("No factors available — check score columns in VRAM")
        sys.exit(1)

    # ── Compute point-in-time cap quartile masks ──────────────────────────────
    log.info("Computing point-in-time market cap quartile masks...")
    t_mask = time.time()
    quartile_masks = pack.cap_quartile_masks()
    log.info(f"  Quartile masks computed in {time.time() - t_mask:.1f}s")

    # Log tier universe sizes for first/last period
    import numpy as np
    for tier_name, mask in quartile_masks.items():
        mask_cpu = cp.asnumpy(mask)
        first_n = mask_cpu[0].sum()
        last_n  = mask_cpu[-1].sum()
        mid_idx = len(mask_cpu) // 2
        mid_n   = mask_cpu[mid_idx].sum()
        log.info(f"  {tier_name}: first_period={first_n} mid={mid_n} last_period={last_n} stocks")
    ...
    for tier_name in tiers_to_run:
        tier_mask = quartile_masks[tier_name]
        log.info(f"\n{'─'*60}")
        log.info(f"  TIER: {tier_name} — {len(factors_in_vram)} factors")
        log.info(f"{'─'*60}")
        ...
        results = run_portfolio_batch_gpu(
            ...
            cap_tier      = tier_name,
            cap_mask_gpu  = tier_mask,
            cycle_tag     = "CYC-009-PM",
            save          = not args.dry_run,
            overwrite     = False,   # skip existing — avoid slow UPDATE on huge JSON blobs
            cb            = _cb,
        )

        tier_s = time.time() - t_tier
        ok = sum(1 for r in results if r.get("status") == "complete")
        all_results[tier_name] = results
        grand_total += ok
        log.info(f"  {tier_name}: {ok}/{len(factors_in_vram)} OK in {tier_s:.0f}s")

    # ── Summary ───────────────────────────────────────────────────────────────
    total_s = time.time() - t0

    print(f"\n{'='*70}")
    print(f"  CYC-009 COMPLETE | {grand_total} total OK | {total_s:.0f}s total")
    print(f"{'='*70}")
    ...
    for tier_name, results in all_results.items():
        ...
        print(f"\n  -- {tier_name} (top 10) --")
        print(f"  {'Factor':<35} {'OBQ Port':>8} {'CAGR':>7} {'Calmar':>7} "
    ...
    # Save run log
    if not args.dry_run:
        log_path = "cyc009_portfolio_results.log"
        with open(log_path, "w", encoding="utf-8") as f:
            f.write(f"CYC-009 Cap-Tier Portfolio Run — {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Tiers: {', '.join(tiers_to_run)} | Total: {grand_total} OK | {total_s:.0f}s\n\n")
            for tier_name, results in all_results.items():
                f.write(f"\n=== {tier_name} ===\n")
                for r in results:
                    pm = r.get("portfolio_metrics", {})
